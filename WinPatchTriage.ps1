...
function Write-Log {
  ...
  Write-Write-Log -Message $Message -Level $Level
}
...
function Get-Preflight {
  ...
  return @{
    ...
    DiskFreeOk = $diskOk
    Notes = @(
      $(if(-not $diskOk){"Low disk space on output drive (<200MB) may cause bundle failures."} else {""})
      $(if(-not $isAdmin){"Not running as Administrator - event log export may be incomplete."} else {""})
...
function Get-OsInfo {
  ...
  $installDate = $null
  try {
    $installDate = (Get-Date "1970-01-01Z").AddSeconds([int64]$cv.InstallDate).ToString("o")
  } catch {}
  $os = @{
    ...
    DisplayVersion = ($cv.DisplayVersion | ForEach-Object { $_ })
...
function Write-HtmlReport([hashtable]$Report, [string]$Path) {
  ...
  $msmqClass = if ($msmq.Present) { "warn" } else { "ok" }
  $sibClass  = if ($sib.Present) { "warn" } else { "ok" }
  $msmqServiceValue = if ($msmq.Service) { "$($msmq.Service.Status) / $($msmq.Service.StartType)" } else { "Not found" }
  $msmqServiceEscaped = Html-Escape $msmqServiceValue
  $msmqFeatureStateEscaped = Html-Escape ([string]$msmq.FeatureState)
...
<body>
  ...
  <div class="$msmqClass">
    ...
    <div><strong>FeatureState</strong>: $msmqFeatureStateEscaped</div>
    <div><strong>Service</strong>: $msmqServiceEscaped</div>
